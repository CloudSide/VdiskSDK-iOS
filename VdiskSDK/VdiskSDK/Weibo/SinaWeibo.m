         ...
         andDelegate:(id<SinaWeiboDelegate>)_delegate
{
    if ((self = [super init]))
    {
        ...
        if (!_ssoCallbackScheme)
        {
            ...
            //_ssoCallbackScheme = [NSString stringWithFormat:@"sinavdisksso.%@://", self.appKey];
        }
...
- (void)logInDidFinishWithAuthInfo:(NSDictionary *)authInfo
{
    ...
    if (access_token && uid)
    {
        ...
        [[VdiskSession sharedSession] authorize:nil didSucceedWithAccessToken:access_token refreshToken:refresh_token userID:uid expiresIn:[self.expirationDate timeIntervalSince1970]];
    ...
    {
        NSString *error_code = [authInfo objectForKey:@"error_code"];
        
        if (error_code)
        {
            NSString *error_description = [authInfo objectForKey:@"error_description"];
            NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:
                                      authInfo, @"error",
            ...
            [[VdiskSession sharedSession] authorize:nil didFailWithError:error];
...
- (void)logIn
{
    if ([self isAuthValid])
    {
        ...
        [[VdiskSession sharedSession] authorize:nil didSucceedWithAccessToken:self.accessToken refreshToken:self.refreshToken userID:self.userID expiresIn:[self.expirationDate timeIntervalSince1970]];
